...
- (void)changeState {
    ...
    // 非空dayView的状态
        ...
        if ([_manager.helper date:_date isTheSameDayThan:_manager.date] && self.enabled) {
            [self setImage:[[UIImage imageNamed:@"circle"] imageWithRenderingMode:_manager.imageRenderingMode] forState:UIControlStateNormal];
            [self setTitle:@"今天" forState:UIControlStateNormal];
            self.titleLabel.font = [UIFont systemFontOfSize:14.f];
            [self setTitleColor:[UIColor wheatColor] forState:UIControlStateNormal];
        } else {
            [self setImage:nil forState:UIControlStateNormal];
        }
